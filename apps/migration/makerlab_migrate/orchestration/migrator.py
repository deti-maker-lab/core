...
def _generate_model_legacy_id(codigo: str) -> int:
    ...
    # Truncate to fit within PostgreSQL BIGINT (signed 64-bit: -9223372036854775808 to 9223372036854775807)
    normalized = normalize_codigo(codigo).encode('utf-8')
    hash_val = hashlib.md5(normalized).hexdigest()
    # Take first 15 hex digits (60 bits) to stay well within 63-bit signed integer range
    truncated_hash = hash_val[:15]
            ...
            for i in range(0, len(users), batch_size):
                ...
                for legacy_user in batch:
                    try:
                        ...
                        # Generate placeholder email if missing (to avoid unique constraint violations)
                        email = legacy_user.email
                        if not email:
                            email = f"user_{legacy_user.id}@legacy.local"
                        ...
                        stat_key = {"INSERT": "inserted", "UPDATE": "updated"}.get(action, action.lower())
                        self.stats["users"][stat_key] += 1
            ...
            for i in range(0, len(projects), batch_size):
                ...
                for legacy_project in batch:
                    try:
                        ...
                        # Map action to stat key (past tense)
                        stat_key = {"INSERT": "inserted", "UPDATE": "updated"}.get(action, action.lower())
                        self.stats["projects"][stat_key] += 1
                        self.logger.log_entity_action(
                        ...
                        # Map action to stat key (past tense)
                        stat_key = {"INSERT": "inserted", "UPDATE": "updated"}.get(owner_action, owner_action.lower())
                        self.stats["project_members"][stat_key] += 1
                        ...
                        for member in legacy_project.members:
                            ...
                            # Map action to stat key (past tense)
                            stat_key = {"INSERT": "inserted", "UPDATE": "updated"}.get(member_action, member_action.lower())
                            self.stats["project_members"][stat_key] += 1
            ...
            for i in range(0, len(codigos_list), batch_size):
                ...
                for codigo, equipment_group in batch:
                    try:
                        ...
                        self.stats["equipment_models"][stat_key] += 1
                        self.logger.log_entity_action(
                        ...
                        for idx, legacy_eq in enumerate(equipment_group):
                            ...
                            for q in range(quantity):
                                ...
                                # Map action to stat key (past tense)
                                stat_key = {"INSERT": "inserted", "UPDATE": "updated"}.get(eq_action, eq_action.lower())
                                self.stats["equipment"][stat_key] += 1
                                self.logger.log_entity_action(
